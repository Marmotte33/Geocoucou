...
class GPXProcessor:
    ...
    def get_emoji_for_icon(self, icon: str, waypoint_name: str = "") -> str:
        ...
        icon_mapping = {
            ...
            'cycling': '🚴', 'vélo': '🚴', 'velo': '🚴', 'bicycle': '🚴', 'bike': '🚴',
            'mountain_biking': '🚵', 'vtt': '🚵', 'mtb': '🚵', 'mountain_bike': '🚵',
            'surfing': '🏄', 'surf': '🏄', 'surfing_spot': '🏄',
            'kitesurfing': '🪁', 'kite': '🪁', 'kitesurf': '🪁',
            'windsurfing': '🏄', 'windsurf': '🏄', 'planche': '🏄',
            'sailing': '⛵', 'voile': '⛵', 'yacht': '⛵', 'sailboat': '⛵',
            'kayaking': '🛶', 'kayak': '🛶', 'canoe': '🛶', 'canoeing': '🛶',
            'rowing': '🚣', 'aviron': '🚣', 'rowing_club': '🚣',
            'diving': '🤿', 'plongée': '🤿', 'plongee': '🤿', 'scuba': '🤿',
            'snorkeling': '🤿', 'snorkel': '🤿', 'apnée': '🤿',
            'horse_riding': '🏇', 'équitation': '🏇', 'equitation': '🏇', 'horse': '🏇',
            'equestrian': '🏇', 'riding': '🏇', 'cheval': '🏇',
            'boxing': '🥊', 'boxe': '🥊', 'boxing_club': '🥊',
            'martial_arts': '🥋', 'arts_martiaux': '🥋', 'karate': '🥋',
            'judo': '🥋', 'taekwondo': '🥋', 'aikido': '🥋',
            'wrestling': '🤼', 'lutte': '🤼', 'wrestling_club': '🤼',
            'weightlifting': '🏋️', 'musculation': '🏋️', 'bodybuilding': '🏋️',
            'powerlifting': '🏋️', 'haltérophilie': '🏋️',
            'gymnastics': '🤸', 'gymnastique': '🤸', 'gym': '🤸',
            'dance': '💃', 'danse': '💃', 'dancing': '💃', 'ballet': '💃',
            'ice_skating': '⛸️', 'patinage': '⛸️', 'patin': '⛸️', 'skating': '⛸️',
            'hockey': '🏒', 'hockey_sur_glace': '🏒', 'hockey_club': '🏒',
            'ice_hockey': '🏒', 'field_hockey': '🏑',
            'baseball': '⚾', 'baseball_field': '⚾', 'baseball_club': '⚾',
            'softball': '🥎', 'softball_field': '🥎',
            'cricket': '🏏', 'cricket_field': '🏏', 'cricket_club': '🏏',
            'rugby': '🏉', 'rugby_club': '🏉', 'rugby_field': '🏉',
            'american_football': '🏈', 'football_americain': '🏈',
            'handball': '🤾', 'handball_club': '🤾', 'handball_court': '🤾',
            'badminton': '🏸', 'badminton_court': '🏸', 'badminton_club': '🏸',
            'table_tennis': '🏓', 'ping_pong': '🏓', 'tennis_de_table': '🏓',
            'squash': '🏓', 'squash_court': '🏓', 'squash_club': '🏓',
            'racquetball': '🏓', 'racquetball_court': '🏓',
            'padel': '🏓', 'padel_court': '🏓', 'padel_club': '🏓',
            'paddle': '🏓', 'paddle_court': '🏓',
            'archery': '🏹', 'tir_à_l_arc': '🏹', 'tir_arc': '🏹', 'archery_range': '🏹',
            'shooting': '🎯', 'tir': '🎯', 'shooting_range': '🎯',
            'paintball': '🎯', 'paintball_field': '🎯',
            'airsoft': '🎯', 'airsoft_field': '🎯',
            'skateboarding': '🛹', 'skate': '🛹', 'skateboard': '🛹', 'skatepark': '🛹',
            'roller_skating': '🛼', 'roller': '🛼', 'patin_à_roulettes': '🛼',
            'inline_skating': '🛼', 'rollerblading': '🛼',
            'bmx': '🚲', 'bmx_track': '🚲', 'bmx_park': '🚲',
            'motocross': '🏍️', 'mx': '🏍️', 'motocross_track': '🏍️',
            'motorcycle': '🏍️', 'moto': '🏍️', 'motorcycle_track': '🏍️',
            'formula1': '🏎️', 'f1': '🏎️', 'formula_1': '🏎️', 'racing': '🏎️',
            'nascar': '🏎️', 'stock_car': '🏎️', 'race_track': '🏎️',
            'rally': '🏎️', 'rallye': '🏎️', 'rally_track': '🏎️',
            'karting': '🏎️', 'kart': '🏎️', 'karting_track': '🏎️',
            'go_karting': '🏎️', 'go_kart': '🏎️',
            'paragliding': '🪂', 'parapente': '🪂', 'paraglide': '🪂',
            'hang_gliding': '🪂', 'deltaplane': '🪂', 'delta_plane': '🪂',
            'skydiving': '🪂', 'parachute': '🪂', 'parachutisme': '🪂',
            'bungee_jumping': '🪂', 'saut_élastique': '🪂', 'bungee': '🪂',
            'rock_climbing': '🧗', 'escalade': '🧗', 'climbing': '🧗',
            'bouldering': '🧗', 'bloc': '🧗', 'boulder': '🧗',
            'via_ferrata': '🧗', 'via_ferrata': '🧗', 'ferrata': '🧗',
            'canyoning': '🏞️', 'canyon': '🏞️', 'canyoning_spot': '🏞️',
            'rafting': '🛶', 'raft': '🛶', 'rafting_spot': '🛶',
            'white_water': '🛶', 'eau_vive': '🛶', 'eau_vive': '🛶',
            'triathlon': '🏊', 'triathlon_club': '🏊', 'triathlon_center': '🏊',
            'duathlon': '🏃', 'duathlon_club': '🏃', 'duathlon_center': '🏃',
            'ironman': '🏊', 'iron_man': '🏊', 'ironman_triathlon': '🏊',
            'ultra_trail': '🥾', 'ultra': '🥾', 'ultra_trail_race': '🥾',
            'trail_running': '🥾', 'trail': '🥾', 'trail_race': '🥾',
            'orienteering': '🧭', 'course_orientation': '🧭', 'orientation': '🧭',
            'adventure_race': '🏃', 'course_aventure': '🏃', 'aventure': '🏃',
            'obstacle_course': '🏃', 'parcours_obstacles': '🏃', 'obstacles': '🏃',
            'parkour': '🤸', 'freerunning': '🤸', 'parkour_spot': '🤸',
            'free_running': '🤸', 'freerun': '🤸',
            'crossfit': '💪', 'crossfit_box': '💪', 'crossfit_gym': '💪',
            'functional_training': '💪', 'entraînement_fonctionnel': '💪',
            'pilates': '🧘', 'pilates_studio': '🧘', 'pilates_club': '🧘',
            'aerobic': '💃', 'aérobic': '💃', 'aerobic_class': '💃',
            'zumba': '💃', 'zumba_class': '💃', 'zumba_studio': '💃',
            'spinning': '🚴', 'cyclisme_indoor': '🚴', 'spinning_class': '🚴',
            'indoor_cycling': '🚴', 'vélo_indoor': '🚴',
            'aqua_fitness': '🏊', 'aquagym': '🏊', 'aqua_aerobic': '🏊',
            'water_polo': '🤽', 'water_polo_club': '🤽', 'polo_natation': '🤽',
            'synchronized_swimming': '🏊', 'natation_synchronisée': '🏊', 'natation_synchro': '🏊',
            'open_water_swimming': '🏊', 'natation_eau_libre': '🏊', 'eau_libre': '🏊',
            'ice_swimming': '🏊', 'natation_glace': '🏊', 'natation_hiver': '🏊',
            'winter_sports': '❄️', 'sports_hiver': '❄️', 'sports_d_hiver': '❄️',
            'snowboarding': '🏂', 'snowboard': '🏂', 'snowboard_park': '🏂',
            'snowshoeing': '🥾', 'raquettes': '🥾', 'raquettes_à_neige': '🥾',
            'cross_country_skiing': '⛷️', 'ski_de_fond': '⛷️', 'ski_fond': '⛷️',
            'alpine_skiing': '⛷️', 'ski_alpin': '⛷️', 'ski_alpin': '⛷️',
            'freestyle_skiing': '⛷️', 'ski_freestyle': '⛷️', 'freestyle': '⛷️',
            'biathlon': '⛷️', 'biathlon_club': '⛷️', 'biathlon_center': '⛷️',
            'curling': '🥌', 'curling_club': '🥌', 'curling_center': '🥌',
            'bobsleigh': '🛷', 'bobsled': '🛷', 'bobsleigh_track': '🛷',
            'skeleton': '🛷', 'skeleton_track': '🛷', 'skeleton_club': '🛷',
            'luge': '🛷', 'luge_track': '🛷', 'luge_club': '🛷',
            'ice_climbing': '🧗', 'escalade_glace': '🧗', 'ice_climbing_spot': '🧗',
            'ice_fishing': '🎣', 'pêche_glace': '🎣', 'pêche_sur_glace': '🎣',
            'ice_hockey': '🏒', 'hockey_sur_glace': '🏒', 'hockey_glace': '🏒',
